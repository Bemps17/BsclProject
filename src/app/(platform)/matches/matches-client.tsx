      ...
      {matches.length === 0 ? (
        ...
        <TableScroll minWidth={560}>
          ...
          <tbody>
            {matches.map((m) => (
              <tr key={m.id}>
                <td className="border-b border-border px-2.5 py-2.5 font-[family-name:var(--font-jetbrains)] text-[11px] text-muted-foreground">
                  #{String(m.number).padStart(3, "0")}
                </td>
                <td className="border-b border-border px-2.5 py-2.5">{m.alphaName}</td>
                <td
                  className={`border-b border-border px-2.5 py-2.5 font-[family-name:var(--font-jetbrains)] font-bold ${
                    m.status === "CONFIRMED"
                      ? m.alphaWon
                        ? "text-chart-2"
                        : "text-destructive"
                ...
                <td className="border-b border-border px-2.5 py-2.5">{m.bravoName}</td>
